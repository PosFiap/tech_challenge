import { EStatus, HttpRequest, HttpResponse, IHttp } from '../../modules/pagamento'
...
export class HttpClientMock implements IHttp {
  async request<T>(_config: HttpRequest): Promise<HttpResponse<T>> {
    ...
    await pedidoRepository.atualizarStatusPedidoPago(parseInt(_config.body.external_reference, 10), EStatus['Pedido em preparação'])
