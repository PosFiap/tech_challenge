...
export class PedidoService implements IRegistraPedidoUseCase, IListaPedidosUseCase, IAtualizaStatusPedidoUseCase {
  ...
  async atualizaStatus (data: AtualizaStatusPedidoDTO): Promise<AtualizaStatusPedidoOutputDTO> {
    ...
    try {
      ...
      if (statusAtualDoPedido >= codigoStatus) { throw new CustomError(CustomErrorType.BusinessRuleViolation, 'O status indicado não é válido para esse pedido') }
  ...
  async listaPedidos (): Promise<ItemListaPedidoOutputDTO[]> {
    ...
    const listaPedidos: ItemListaPedidoOutputDTO[] =
            pedidosArmazenados.map((pedido: Pedido) => {
              const itensDePedido: ItemPedidoListaPedidoOutputDTO[] =
